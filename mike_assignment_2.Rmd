...
Question 3:
```{r plot t}
ggplot(data = test.df, aes(tval, group = b1, col = b1))+
  geom_density() + xlab("t value") + ylab("Density")+
  facet_grid(sd.err~n)

ggplot(data = test.df, aes(as.factor(n), tval))+
  geom_boxplot()+
  facet_grid(b1~sd.err, scales = "free")
```
